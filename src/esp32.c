...
static enum ble_device_addr read_device_address(uint8_t addr[BLE_ADDR_LEN])
{
	...
	if (rc != 0) {
		PBLE_LOG_ERROR("error determining address type; rc=%d", rc);
		PBLE_ASSERT(0);
	}

	rc = ble_hs_id_copy_addr(t.type, t.val, NULL);
	if (rc != 0) {
		PBLE_LOG_ERROR("error reading address; rc=%d", rc);
		PBLE_ASSERT(0);
	}
...
static void on_gatt_register(struct ble_gatt_register_ctxt *ctxt, void *arg)
{
	...
	case BLE_GATT_REGISTER_OP_SVC:
		PBLE_LOG_DEBUG("registered service %s with handle=%d\n",
				ble_uuid_to_str(ctxt->svc.svc_def->uuid, buf),
				ctxt->svc.handle);
		break;
	case BLE_GATT_REGISTER_OP_CHR:
		PBLE_LOG_DEBUG("registered characteristic %s with "
				"def_handle=%d val_handle=%d\n",
				ble_uuid_to_str(ctxt->chr.chr_def->uuid, buf),
				ctxt->chr.def_handle, ctxt->chr.val_handle);
		break;
	case BLE_GATT_REGISTER_OP_DSC:
		PBLE_LOG_DEBUG("registered descriptor %s with handle=%d\n",
...
{
	PBLE_LOG_INFO("reset reason: %d", reason);
}
...
static int adv_start(struct ble *self)
{
	...
	if (rc != 0) {
		PBLE_LOG_ERROR("adv failure: %d", rc);
		return -EFAULT;
	}
		...
		bool primary, uint8_t nr_chrs)
{
	...
	PBLE_ASSERT(memsize > (uint16_t)(sizeof(struct ble_gatt_service) +
	...
	PBLE_ASSERT(uuid != NULL);
	PBLE_ASSERT(uuid_len == 2 || uuid_len == 4 || uuid_len == 16);
...
{
	PBLE_ASSERT(svc->characteristics.index < svc->characteristics.nr_max);
...
struct ble *esp32_ble_create(void)
{
...
void esp32_ble_destroy(struct ble *self)
{
